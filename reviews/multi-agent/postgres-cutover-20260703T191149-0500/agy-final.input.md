Final focused review for BAYESIANQC local/dev Postgres cutover. Only verify the previously open P1s: run_demo Postgres readiness race, MIGRATION_STRATEGY bare Postgres test command, disposable copy DB provisioning, and destructive copy target guard. Findings first; state whether any P0/P1 remains for local/dev. Scope excludes shared-lab production. Do not mutate files.
