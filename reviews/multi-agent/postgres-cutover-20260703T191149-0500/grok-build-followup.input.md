Review the remediated BAYESIANQC local/dev Postgres cutover for build/runtime/test failure modes and operational gaps. Findings first with P0/P1/P2 severity. Treat P0/P1 as blocking for the local/dev cutover unless explicitly scoped to shared-lab production. Verify prior blockers: alembic.ini default, dual migrations, posterior recomputation, downgrade tests, runtime on 8010, and copy target. Do not mutate files.
