Review the remediated BAYESIANQC local/dev Postgres cutover. Findings first with P0/P1/P2 severity. Treat P0/P1 as blocking for the local/dev cutover unless the packet explicitly scopes them to shared-lab production. Verify prior blockers: stale port 8010 runtime, alembic.ini SQLite default, AGENTS.md SQLite canonicalization, destructive copy rehearsal target, posterior parameter recomputation, downgrade/re-upgrade coverage, and validation wording. Do not mutate files.
