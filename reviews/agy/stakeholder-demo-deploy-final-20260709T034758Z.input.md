# BAYESIANQC stakeholder demo deployment final rereview packet
Scope: final P0/P1 rereview for password-protected synthetic-data demo deploy path at qc.geoffsmiscellany.com.
Validation after latest fixes: bash -n scripts/demo_vps.sh deploy/demo-vps/remote.sh; pytest tests/test_deployment_runtime.py (11 passed); Caddy validate with generated hash and http://caddy site label; make lint; make typecheck; make build; git diff --check.
Latest fixes: Caddyfile now matches public domain and internal http://caddy; rollback no longer runs old Alembic migrations and links target release before smoke so reset-data can realign disposable schema if needed; docs state API docs are intentionally not exposed, rollback crossing migrations requires reset-data, shared edge-admin audit attribution, SSH host-key prerequisite, and image pruning boundary.
Previous fixed findings retained: bcrypt hash escaped as $$ for Compose env_file; Caddy rotation force-recreates Caddy; archive cleanup runs inside API container; public smoke runs in local wrapper; release images tagged by BAYESIANQC_RELEASE_ID and rollback rebuilds only if tags are missing.
Please report only remaining P0/P1 blockers. Treat dirty-worktree deployment refusal as intentional and documented.
...
- FastAPI `/docs`, `/redoc`, and `/openapi.json` are intentionally not exposed
  through the public demo proxy; the demo is a UI walkthrough, not an API
  sandbox.
